 ...
 * ntp_client.c
 *
 * Minimal SNTP client (RFC 5905 subset) -> query server, print offset/delay in ms.
 *
 * Build:
...
static int64_t ntp_ts_to_unix_ms(const uint8_t *buf) {
    ...
    uint64_t usec = ((uint64_t)frac * 1000000ULL) >> 32; /* fraction to microseconds */
...
static void build_ntp_request(uint8_t *buf) {
    ...
    /* LI = 0 (no warning), VN = 4 (version), Mode = 3 (client) -> 0b00 100 011 = 0x23 */
...
static int do_ntp_query(const char *server, int timeout_ms,
                        int64_t *out_local_before_ms,
                        int64_t *out_remote_ms,
                        int64_t *out_local_after_ms,
                        char *server_addr_str, size_t server_addr_len)
{
    ...
    for (rp = res; rp != NULL; rp = rp->ai_next) {
        ...
        ssize_t sent = sendto(sock, packet, NTP_PACKET_SIZE, 0, rp->ai_addr, rp->ai_addrlen);
        ...
        ssize_t rec = recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr*)&src, &srclen);
        ...
        *out_remote_ms = remote_ms;
        ...
        if (server_addr_str && server_addr_len > 0) {
            if (src.ss_family == AF_INET) {
                ...
                inet_ntop(AF_INET, &s4->sin_addr, server_addr_str, server_addr_len);
            } else if (src.ss_family == AF_INET6) {
                struct sockaddr_in6 *s6 = (struct sockaddr_in6*)&src;
                inet_ntop(AF_INET6, &s6->sin6_addr, server_addr_str, server_addr_len);
            } else {
...
void usage(const char *prog) {
    printf("Usage: %s [-t timeout_ms] [-r retries] [-n] [-v] [-s] [-h] [ntp server]\n", prog);
 ...
 * 1. Parse command-line arguments for server, timeout, retries, and syslog option.
 ...
 * 5. On success:
 *    - Calculate average local time, offset (remote - local), and roundtrip delay.
 *    - Format and print results (server, remote time, timings, offset, roundtrip).
...
int main(int argc, char **argv) {
    ...
    if (verbose) {
        ...
        printf("Timeout: %d ms, Retries: %d, Syslog: %s\n", timeout_ms, retries, use_syslog ? "on" : "off");
    }
    ...
    for (attempt = 0; attempt < retries; ++attempt) {
        if (do_ntp_query(server, timeout_ms, &local_before_ms, &remote_ms, &local_after_ms, server_addr, sizeof(server_addr)) == 0) {
            success = 1;
            break;
        }
    ...
    if (!success) {
        fprintf(stderr, "ERR: Failed to contact NTP server %s after %d attempts\n", server, retries);
        if (use_syslog) {
            syslog(LOG_ERR, "NTP query failed for %s after %d attempts", server, retries);
            closelog();
        }
        return 2;
    }
    ...
    /* estimate network delay and clock offset like simple SNTP:
       ...
       For a simple client we use offset = ((t3 + t2) - (t1 + t4)) / 2,
       but since we don't know server receive time, we use offset ≈ t3 - ((t1 + t4) / 2)
    */
    ...
    if (use_syslog) {
        syslog(LOG_INFO, "NTP server=%s addr=%s offset_ms=%lld rtt_ms=%lld", server, server_addr, (long long)epoch_diff_ms, (long long)roundtrip_ms);
        closelog();
    }
    ...
    if(getuid() != 0 && !test_only) {
        ...
        if (use_syslog) {
            openlog("ntp_client", LOG_PID|LOG_CONS, LOG_USER);
            syslog(LOG_WARNING, "Not running as root, cannot set system time");
            closelog();
        }
        return 0;
    }
    ...
    if(!test_only) {
        ...
        if (llabs(epoch_diff_ms) > 0 && llabs(epoch_diff_ms) < 500) {
            ...
            if (adjtime(&delta, NULL) == 0) {
                fprintf(stderr,"INF: System time adjusted by %lld ms using adjtime.\n", (long long)epoch_diff_ms);
                if (use_syslog) syslog(LOG_INFO, "System time adjusted by %lld ms using adjtime", (long long)epoch_diff_ms);
            } else {
                perror("adjtime");
                if (use_syslog) syslog(LOG_ERR, "Failed to adjust system time: %s", strerror(errno));
            }
            } else {
            fprintf(stderr,"INF: Remote year is less than 2025, not adjusting system time.\n");
            if (use_syslog) syslog(LOG_WARNING, "Remote year < 2025, not adjusting system time");
            }
        } else {
            fprintf(stderr,"Delta is not within 500ms, not setting system time.\n");
            if (use_syslog) syslog(LOG_WARNING, "Delta not within 500ms, not setting system time");
        }
    }
    return 0;
}
